...
界面窗口的根布局是`DecorView`，该类继承自`FrameLayout`，而`FrameLayout`继承自`ViewGroup`。感觉绘制肯定会在`ViewGroup`或者`View`中
...
`View`绘制的三个过程Measure、Layout、Draw 
...
```
`getDefaultSize`方法又会使用到`MeasureSpec`类
...
`getWidth()`与`getMeasuredWidth()`有什么区别呢？                          
